...
object ex311 {
  ...
  object List {
    ...
    def sum[A](as: List[A])(implicit ev: Numeric[A]): A =
      foldLeft(as, ev.zero)((acc, x) => ev.plus(acc, x))

    def product[A](as: List[A])(implicit ev: Numeric[A]): A =
      foldLeft(as, ev.one)((acc, x) => ev.times(acc, x))

    def reverse[A](as: List[A]): List[A] =
      foldLeft[A, List[A]](as, Nil)((acc, x) => Cons(x, acc))

    def append[A](as: List[A], x: A): List[A] =
      foldRight(as, Cons(x, Nil))((h, acc) => Cons(h, acc))
